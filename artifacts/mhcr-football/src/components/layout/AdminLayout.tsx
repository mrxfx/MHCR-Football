        ...
        <div className="flex items-center justify-between">
          <Link href="/admin/dashboard" onClick={onClose}>
            <div className="flex items-center gap-3 cursor-pointer group">
              <div className="w-9 h-9 rounded-xl bg-blue-500 flex items-center justify-center shadow-lg group-hover:bg-blue-400 transition-colors">
                <Shield size={18} className="text-white" />
              </div>
              <div>
                <p className="text-white font-bold text-sm leading-tight">MHCR Football™</p>
                <p className="text-blue-300 text-xs">Admin Panel</p>
              </div>
            </div>
          </Link>
          {onClose && (
            <button onClick={onClose} className="text-white/60 hover:text-white transition-colors md:hidden">
              <X size={20} />
            </button>
          )}
        </div>
  ...
  return (
    <div className="flex min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-72 bg-[#0f1729] shadow-2xl transform transition-transform duration-300 md:hidden ${mobileOpen ? "translate-x-0" : "-translate-x-full"}`}>
        <SidebarContent location={location} signOut={signOut} onClose={() => setMobileOpen(false)} />
      </aside>
